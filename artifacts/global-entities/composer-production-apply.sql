-- Byelingua Phase 3.5R manual Composer apply SQL.
-- Fresh snapshot: 2026-08-20T13:23:32.397Z. Do not execute automatically.
BEGIN;
DO $$
BEGIN
 IF (SELECT count(*) FROM public.composers) <> 285 OR (SELECT count(*) FROM public.composer_aliases) <> 66 THEN RAISE EXCEPTION 'Composer master row counts changed since preflight'; END IF;
 IF EXISTS (SELECT 1 FROM (VALUES ('Arturo Márquez', 'composer:35596264768e193f73a6464f37ca0552'),
        ...
        ('Ottorino Respighi', 'composer:245e242a641d08377d24e984e21ad587')) a(canonical_name,identity_key) JOIN public.composers c ON c.identity_key=a.identity_key WHERE c.canonical_name<>a.canonical_name) THEN RAISE EXCEPTION 'Composer identity_key collision'; END IF;
 IF EXISTS (SELECT 1 FROM (VALUES ('Arturo Márquez', 'composer:35596264768e193f73a6464f37ca0552'),
        ('Andrés Villamil', 'composer:c2ac784233aba86f02c760ab322d7f10'),
        ('Alejandro Vivas Puig', 'composer:1f3523c25653c570751b9fc9183e7990'),
        ('Agustín Pío Barrios', 'composer:df4328c7761cf640961a870f207b98ae'),
        ...
        ('Mieczysław Weinberg', 'composer:d034fb4b61c314fd7126bc86fef9aad9'),
        ('Nuria Núñez Hierro', 'composer:ab6c5f55a441baf1c3b07ac9291ce61b'),
        ('Ottorino Respighi', 'composer:245e242a641d08377d24e984e21ad587')) a(canonical_name,identity_key) JOIN public.composers c ON c.canonical_name=a.canonical_name WHERE c.identity_key<>a.identity_key) THEN RAISE EXCEPTION 'Composer canonical-name collision'; END IF;
 IF EXISTS (SELECT 1 FROM (VALUES ('c692545f-1e0e-45ab-aa1a-c4d20224b6cc'::uuid, 'A. de Cabezón', '6a12bc7a-ae8b-49a4-bd00-8fced75a3359'::uuid),
        ...
        ('8196aa4d-463c-4cf1-8103-645698e2d6cb'::uuid, 'Woodkid', '88232afc-3a1d-48a3-ae12-5421449ab9b4'::uuid)) s(id,alias,composer_id) WHERE NOT EXISTS (SELECT 1 FROM public.composer_aliases ca WHERE ca.id=s.id AND ca.alias=s.alias AND ca.composer_id=s.composer_id)) THEN RAISE EXCEPTION 'Alias master changed since preflight'; END IF;
 IF EXISTS (SELECT 1 FROM (VALUES ('Arturo Márquez', 'A. Márquez'),
        ...
        ('Ottorino Respighi', 'Respighi')) a(canonical_name,alias) JOIN public.composer_aliases ca ON ca.alias=a.alias JOIN public.composers c ON c.id=ca.composer_id WHERE c.canonical_name<>a.canonical_name) THEN RAISE EXCEPTION 'Exact staged alias collision'; END IF;
END $$;
WITH approved(canonical_name,identity_key) AS (VALUES ('Arturo Márquez', 'composer:35596264768e193f73a6464f37ca0552'),
        ('Andrés Villamil', 'composer:c2ac784233aba86f02c760ab322d7f10'),
        ('Alejandro Vivas Puig', 'composer:1f3523c25653c570751b9fc9183e7990'),
        ('Agustín Pío Barrios', 'composer:df4328c7761cf640961a870f207b98ae'),
        ('Gustav Holst', 'composer:bbc1073a0072b7007aa253af9fb9182f'),
