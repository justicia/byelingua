...
def run_pipeline(*, venue: str, season: str, mode: str = "dry-run", output_dir: Path = Path("season-ingestion-output"), snapshot_path: Path | None = None) -> dict[str, Any]:
    ...
    payloads = {"source_audit": {"venue": venue, "season": season, "official_source": config["official_source"], "adapter_errors": adapter.last_errors, "events": len(events)}, "raw": [event.raw | {"event_key": event.event_key, "source_url": event.source_url} for event in events], "normalized": [event.to_dict() for event in events], "snapshot": snapshot.__dict__, "resolution_staging": resolution_rows, "final_staging": {"events": [event.to_dict() for event in events], "resolution": resolution_rows, "review": review_rows, "writes": 0}, "summary": {"generated_at": generated_at, "venue": venue, "season": season, "mode": mode, "counts": {"events": len(events), "works_existing": sum(row["status"] == "existing" for row in resolution_rows), "works_review": len(review_rows), "review_items": len(review_rows), "writes": 0}, "gates": gates, "passed": all(gates.values())}}
