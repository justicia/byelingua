...
def test_different_source_identities_for_one_event_do_not_interrupt_pagination(monkeypatch):
    monkeypatch.setenv("SUPABASE_URL", "https://database.example")
    monkeypatch.setenv("SUPABASE_READONLY_KEY", "readonly-test-value")
    requests = []
    ...
    def fetcher(request, timeout):
        ...
        if offset >= 2:
            return Response([])
        item = api_row(offset + 1)
        item["event_id"] = "shared-event"
        item["events"]["id"] = "shared-event"
        return Response([item])

    rows = fetch_source_rows(
        "operadeparis", "2026-08-28", "2027-08-31", page_size=1, fetcher=fetcher
    )
    ...
    report = summarize_source(
        "operadeparis", "2026-08-28", "2027-08-31", rows, bounds_source="configured"
    )
    assert report["record_count"] == 2
    assert report["multiple_source_identities_per_event_id"]["count"] == 1
    assert report["failures"][-1]["code"] == "multiple_source_identities_per_event_id"


def test_same_complete_source_row_across_pages_is_pagination_duplicate(monkeypatch):
    monkeypatch.setenv("SUPABASE_URL", "https://database.example")
    monkeypatch.setenv("SUPABASE_READONLY_KEY", "readonly-test-value")
...
def test_many_to_one_source_identities_fail_after_full_summary():
    rows = [row(1, source_id="performance-1"), row(2, source_id="performance-2")]
    rows[1]["event_id"] = rows[0]["event_id"]

    report = summarize_source("test", "2026-09-01", "2027-08-31", rows, bounds_source="default")

    assert report["record_count"] == 2
    assert report["distinct_source_event_ids"] == 2
    assert report["first_date"] == "2026-09-01"
    assert report["last_date"] == "2026-09-01"
    assert report["audit_passed"] is False
    assert report["multiple_source_identities_per_event_id"] == {
        "count": 1,
        "samples": [
            {
                "event_id": "event-1",
                "source_event_ids": ["performance-1", "performance-2"],
                "record_count": 2,
            }
        ],
    }
    assert "multiple_source_identities_per_event_id" in {
        failure["code"] for failure in report["failures"]
    }


def test_pagination_duplicate_row_keeps_specific_failure_code():
    def duplicate(*_args):
        raise AuditReadError("duplicate", code="pagination_duplicate_row")

    report = audit_season_sources("2026-27", CONFIG, sources=["auditorio_nacional"], fetch_rows=duplicate)

    assert report["audit_passed"] is False
    assert report["sources"][0]["failures"] == [
        {"code": "pagination_duplicate_row", "message": "duplicate"}
    ]
