-- Repair migration: resolve Artist independently for every event_credit row.
create or replace function public.apply_canonical_production_graph(p_payload jsonb)
returns jsonb language plpgsql security invoker set search_path = public
as $$
declare
  v_org_id uuid; v_venue_id uuid; v_event_id uuid; v_work_id uuid; v_artist_id uuid; v_key text; v_source_event text; v_role text; v_character text; v_character_id uuid; v_raw_character text; v_result jsonb := '{}'::jsonb;
begin
  ...
  for v_key in select distinct x->>'event_key' from jsonb_array_elements(coalesce(p_payload->'event_credits','[]'::jsonb)) x loop
    ...
    for v_artist_id,v_role,v_character_id,v_character,v_raw_character in
      select coalesce(nullif(x->>'artist_id','')::uuid,
          ...
          x->>'role',
          nullif(x->>'character_id','')::uuid,
          nullif(x->>'character',''),
          nullif(x->>'raw_character','')
      from jsonb_array_elements(p_payload->'event_credits') x
      where x->>'event_key'=v_key
    loop
